...
JWT_SECRET_KEY = os.getenv("FINANCE_JWT_SECRET_KEY")

if not JWT_SECRET_KEY:
    if os.getenv("VERCEL"):
        raise RuntimeError(
            "FINANCE_JWT_SECRET_KEY must be configured in the Vercel environment"
        )
    JWT_SECRET_KEY = "development-secret-change-before-production"
...
def create_access_token(
    ...
    if user_id <= 0:
        raise AuthenticationError("User ID must be greater than zero")
